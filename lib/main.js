...
async function run() {
    try {
        ...
        if (prInfo.reviewState === 'pending' && configObj.onReviewPending) {
            console.log('onReviewPending action triggered!', configObj.onReviewPending);
        ...
        if (prInfo.reviewState === 'changes_requested' &&
            ...
            console.log('onChangeRequest action triggered!', configObj.onChangeRequest);
        ...
        if (prInfo.requested_reviewers.length > 0 && !prInfo.reviewState &&
            configObj.onRequestReview) {
            console.log('onRequestReview action triggered!', configObj.onRequestReview);
            githubAction = configObj.onRequestReview;
        }
        if (!githubAction) {
            console.log('There is no configuration match for this action');
            return;
        }
        console.log('PR current actions based on pull request and review state:', githubAction);
